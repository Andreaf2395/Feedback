...
<html>
	...
	<body>
		...
		<div id="clg_lab_dtls" class="container col-md-8 box ">
			...
			<form action="savefeedback" method="POST"  enctype="multipart/form-data">
				{{csrf_field()}}
					...
					<div class="form-group">
						...
						@endif
						<select name="college"   class="form-control dropdown-toggle" >
							<option></option>
							@foreach($colleges as $c)	
								@if(old('college') == $c->id)
									<option value="{{ $c->id }}" selected class="dropdown-item">{{$c->college_name}},{{$c->district}}</option>
								@else
									<option value="{{ $c->id }}" class="dropdown-item">{{$c->college_name}},{{$c->district}}</option>
								@endif
							@endforeach
						</select>
					...
					<br>
					<div id="form-group">
						<label for="comment">In what way are you using the lab?</label>
								...
								<div id="upload_element" >
									 <span class="col-md-2 ">
											...
											<div class="fileinputs">
												<input type="file" class="file"  id="labimage" name="labimage" value="{{old('labimage')}}"/>
											</div>
		...
		$(document).ready(function() {
                        ...
                        $.ajax({
                            ...
                            success:function(data) {
                                ...
                                if(data.faculty.length== 0)
                                {
                                    ...
                                    $('#msg1').append('<p >NO FACULTY MEMBERS . Please add a Faculty member </p>');
                                }
                                else{
                ...
                function display(data){
                    if(!data.empty)
                    {
                        ...
                        $('#displayboard').append('<div class="col-md-2 " id="lab-pic'+index+'"><img class="img-responsive img-thumbnail" src="'+'/images/'+collegeID+'/Lab/'+data.image_name+'" id="preview_image_'+index+'" /><input type="hidden" value="'+data.image_name+'" id="file_name_'+index+'"><div class="delete_btn" id='+index+' ><img src="{{asset('images/delete.png')}}" /></div></div>');
                            index++;
                    }
